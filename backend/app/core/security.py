...
def verify_token(token: str) -> dict:
    ...
    try:
        ...
        logger.error('JWT verification error: {}', str(e))
        raise HTTPException(
...
def validate_file_type(file: UploadFile) -> str:
    ...
    # Validate detected type - Add "application/vnd.microsoft.portable-executable" as valid for exe files

    if extension == "exe" and not detected_type.startswith((
    ...
    if extension == "pdf" and detected_type != "application/pdf":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"File does not match its extension. Detected: {detected_type}",
        )
...
def save_upload_file(file: UploadFile, file_type: str) -> str:
    ...
    try:
        ...
        logger.error('File save error: {}', str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error saving file"
        )
