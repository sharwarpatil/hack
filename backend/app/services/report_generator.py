...
def generate_report(task_id: str, format: str = "pdf", analysis_data: Optional[Dict[str, Any]] = None) -> str:
    ...
    logger.info('Generating {} report for task {}', format, task_id)
...
def generate_pdf_report(task_id: str, analysis_data: Dict[str, Any]) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.error('Error generating simplified PDF report: {}', str(simplified_error))
...
<div class="section">
    ...
    {% if analysis.dynamic_analysis.network_activity %}
    ...
    <table>
        ...
        {% for conn in analysis.dynamic_analysis.network_activity %}
    ...
    {% if analysis.dynamic_analysis.suspicious_behaviors %}
    ...
    <table>
        ...
        {% for behavior in analysis.dynamic_analysis.suspicious_behaviors %}

        <tr>

            <td>{{ behavior.type }}/{{ behavior.subtype }}</td>

            <td>{{ behavior.description }}</td>

            <td class="severity-{{ behavior.severity }}">{{ behavior.severity|upper }}</td>
    ...
    <!-- More sections for processes, file operations, etc. -->
...
{% endif %}
        ...
        logger.info('HTML report generated: {}', html_path)
...
def generate_json_report(task_id: str, analysis_data: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info('JSON report generated: {}', json_path)
        ...
        logger.error('Error generating JSON report: {}', str(e))
...
def generate_malware_score_chart(malware_score: float, severity: SeverityLevel) -> Optional[str]:
    ...
    try:
        ...
        plt.bar(pos, np.ones_like(pos) * 0.5, width=2 * np.pi / 100, alpha=0.2, color='lightgray')
        ...
        logger.error('Error generating malware score chart: {}', str(e))
