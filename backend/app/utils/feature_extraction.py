...
def extract_features_from_analysis(
    file_type: FileType, analysis_data: Dict[str, Any]
) -> Dict[str, float]:
    """
    ...
    logger.warning('Unsupported file type for feature extraction: {}', file_type)
    return {}
    ...
    try:
        ...
        if sections:
            ...
            features["avg_section_entropy"] = (
                float(sum(section_entropies) / len(sections)) if section_entropies else 0.0
            )
            features["max_section_entropy"] = (
                float(max(section_entropies)) if section_entropies else 0.0
            )
            ...
            executable_writable_count = sum(
                ...
                if "MEM_EXECUTE" in section.get("characteristics", [])
                and "MEM_WRITE" in section.get("characteristics", [])
            )
            ...
            abnormal_section_names = sum(
                1 for section in sections if section.get("name", "") not in normal_section_names
            )
        ...
        features["suspicious_import_count"] = sum(
            features[f"suspicious_{category}_apis"] for category in SUSPICIOUS_API_CALLS.keys()
        )
        ...
        features["total_interesting_strings"] = float(
            url_count + ip_count + registry_count + file_path_count + api_count
        ...
        for import_name in imports:
            if any(
                malware_import.lower() in import_name.lower()
                for malware_import in known_malware_imports
            ):
        ...
        features["has_tls"] = (
            1.0
            if "IMAGE_DIRECTORY_ENTRY_TLS" in str(analysis_data.get("pe_characteristics", {}))
            else 0.0
        )
        ...
        if (
            features["max_section_entropy"] > 7.5
            or features["executable_writable_sections"] > 0
            or features["abnormal_section_names"] > 1
        ):
            has_unusual_sections = 1
        features["has_unusual_sections"] = float(has_unusual_sections)
    except Exception as e:
        logger.error('Error extracting EXE features: {}', str(e))
...
def extract_pdf_features(analysis_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    try:
        ...
        features["has_embedded_files"] = (
            1.0 if analysis_data.get("has_embedded_files", False) else 0.0
        )
        features["has_auto_action"] = 1.0 if analysis_data.get("has_auto_action", False) else 0.0
        features["has_suspicious_objects"] = (
            1.0 if analysis_data.get("has_suspicious_objects", False) else 0.0
        )
        ...
        suspicious_js_count = sum(
            1 for obj in suspicious_objects if obj.get("type", "") == "javascript"
        )
        suspicious_execution_count = sum(
            1 for obj in suspicious_objects if obj.get("type", "") == "execution"
        )
        suspicious_uri_count = sum(1 for obj in suspicious_objects if obj.get("type", "") == "uri")
        suspicious_action_count = sum(
            1 for obj in suspicious_objects if obj.get("type", "") == "action"
        )
        suspicious_embedding_count = sum(
            1 for obj in suspicious_objects if obj.get("type", "") == "embedding"
        )
        ...
        suspicious_url_count = sum(
            ...
            if any(
                ...
                for pattern in ["bit.ly", "tinyurl", "goo.gl", "pastebin", "t.co"]
        ))
        ...
        js_suspicious_patterns = sum(
            js_code_str.count(pattern.lower()) for pattern in SUSPICIOUS_PDF_PATTERNS
        ...
        logger.error('Error extracting PDF features: {}', str(e))
        # Provide default values for critical features
    ...
    # Add to backend/app/utils/feature_extraction.py

def extract_dynamic_features(dynamic_data: Dict[str, Any]) -> Dict[str, float]:
    """
    Extract features from dynamic analysis data
    """
    features = {}
    ...
    try:
        ...
        features["network_connection_count"] = float(len(network_activity))
        
        # Count unique IPs/domains

        unique_destinations = set()
        for conn in network_activity:
            if conn.get("remote_address"):
                unique_destinations.add(conn.get("remote_address"))
        features["unique_network_destinations"] = float(len(unique_destinations))
        
        # Process features

        process_activity = dynamic_data.get("process_activity", [])
        features["process_creation_count"] = float(len(process_activity))
        
        # File system features

        file_activity = dynamic_data.get("file_system_activity", [])
        features["file_operation_count"] = float(len(file_activity))
        
        # Registry features

        registry_activity = dynamic_data.get("registry_activity", [])
        features["registry_operation_count"] = float(len(registry_activity))
        
        # Suspicious behavior counts

        suspicious_behaviors = dynamic_data.get("suspicious_behaviors", [])
        features["suspicious_behavior_count"] = float(len(suspicious_behaviors))
        
        # Count by severity

        high_severity = sum(1 for b in suspicious_behaviors if b.get("severity") == "high")
        medium_severity = sum(1 for b in suspicious_behaviors if b.get("severity") == "medium")
        low_severity = sum(1 for b in suspicious_behaviors if b.get("severity") == "low")
        
        features["high_severity_behaviors"] = float(high_severity)
        features["medium_severity_behaviors"] = float(medium_severity)
        features["low_severity_behaviors"] = float(low_severity)
        
        # Count by type

        network_behaviors = sum(1 for b in suspicious_behaviors if b.get("type") == "network")
        process_behaviors = sum(1 for b in suspicious_behaviors if b.get("type") == "process")
        file_behaviors = sum(1 for b in suspicious_behaviors if b.get("type") == "file_system")
        registry_behaviors = sum(1 for b in suspicious_behaviors if b.get("type") == "registry")
        
        features["suspicious_network_behaviors"] = float(network_behaviors)
        features["suspicious_process_behaviors"] = float(process_behaviors)
        features["suspicious_file_behaviors"] = float(file_behaviors)
        features["suspicious_registry_behaviors"] = float(registry_behaviors)
    except Exception as e:
        logger.error('Error extracting dynamic features: {}', str(e))
    return features